...
int GraphicsView::displayInternCubeThumb( int l )
{
    if( this->cubeStatus == cubeEmpty )
    {
        if( this->loadHypercube() != _OK )
        {
            return _FAILURE;
        }
    }
    ...
    item->setPos(0,0);
    this->setFixedSize( tmpImg.width(), tmpImg.height() );
    this->setVerticalScrollBarPolicy(Qt::ScrollBarAlwaysOff);
    this->setHorizontalScrollBarPolicy(Qt::ScrollBarAlwaysOff);
    return _OK;
}
...
int GraphicsView::loadHypercube()
{
    //-------------------------------------------------
    //Get cube location
    //-------------------------------------------------
    ...
    QString filePath;
    if( funcLetUserSelectFile( &filePath, "Select Hypercube...", this ) != _OK )
    {
        return _FAILURE;
    }
...
QImage GraphicsView::slideImgFromCube( const int &l )
{
    //-------------------------------------------------
